document.addEventListener("DOMContentLoaded", () => {
  ...
  const animateElement = (element, type, callback, duration) => {
    ...
    const animationStep = (timestamp) => {
      ...
      let opacity =
        startOpacity + (progress / duration) * (endOpacity - startOpacity);
  ...
  if (document.querySelector(".main_content")) {
    ...
    const updateRollingText = (activeIndex) => {
      ...
      document
        .querySelector(`.rolling_txt .item_txt:nth-child(${activeIndex + 1})`)
        .classList.add("on");
    };
    main2InitializeSliders();

    let main2Slider1Mobile,
      main2Slider2Mobile,
      main2Slider3Mobile,
      main2Slider4Mobile;
  ...
  const popFn = () => {
    document
      .querySelectorAll(".page_info1 .list_pic > li > a")
      .forEach((link) => {
        link.addEventListener("click", (e) => {
          ...
          popPic.querySelector(".pop_pic .pop_con img").src =
